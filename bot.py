...
if __name__ == "__main__":
    twitterBot = TwitterBot(*tokens['SwagMoneyBot'])
    # twitterBot.start('followbot')
    #twitterBot.start('dmbot')
    #twitterBot.start('mentionbot')
    # twitterBot.start('stopbot-jakesimmonsrock')

    file = open(twitterBot.user.screen_name+"_followers.txt", "a+")

    #remove newline at end of each line
    flwrs = map(lambda x: int(x[:-1]), file.readlines())
    print flwrs

    current = []

    for page in tweepy.Cursor(twitterBot.twitter.api.followers_ids, screen_name="FierceTaint").pages():
        current.extend(page)

    unfollowed = set(flwrs) - set(current)
    print unfollowed
    new_follow = set(current) - set(flwrs)
    print new_follow

    for id in new_follow:
        print "new follow", id
        file.write("%s\n" % id)
    else:
    ...
    for id in unfollowed:
        print "unfollowed you", id
        try:
            print twitterBot.twitter.api.get_user(id)._json['screen_name']
